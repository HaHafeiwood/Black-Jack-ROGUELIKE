...
const ALL_PASSIVES=[
  ...
  {id:'bloodpact', name:'鮮血契約', icon:'📜', cost:null, shop:false, desc:'撿起時最大生命減半；成為後裔前，未來獲得的最大生命也減半。獲得 5 層渴血，戰後無法自然回血；世人評價與信仰項鍊的變化效率減半，只有得到足夠認同時才能向對應教堂祈禱。不可封印、不可出售。成為後裔後進化為血魔契約。'},
...
function enemyAttackEffect(e,explicit=''){
  if(!explicit&&e?.divePending)return '俯衝反擊';
  const action=explicit||e?.inquisitorSync||e?.inquisitorAction||e?.courtAction||e?.cthulhuAction||e?.samuraiAction||e?.kunAction||e?.pengAction||e?.dragonAction||e?.demonAction||e?.paladinAction||e?.mimicAction||e?.werewolfAction||e?.robotAction||e?.cultistAction||e?.gargoyleAction||e?.ninjaAction||e?.skeletonAction||e?.cyclopsAction||e?.zombieAction||e?.batAction;
...
function restoreSave(raw){
  ...
  const warnings=[];
  const seedCode=normalizeSeedCode(src.seedCode||raw.seedCode||`舊存檔-${raw.savedAt||'未知時間'}-${src.floor??src.stage??src.level??0}-${src.gold??src.money??0}`);
  ...
  if(!validRngState(src.rngState))warnings.push('舊版存檔缺少亂數進度，已建立相容種子');
...
function rollEventType(){
  const shopChance=Math.min(1,Math.max(0,G.shopChance||BASE_SHOP_CHANCE));
  if(gameRandom()<shopChance){G.shopChance=BASE_SHOP_CHANCE;return 'shop';}
  if(!G.altarSeen&&gameRandom()<BLOOD_ALTAR_CHANCE){G.altarSeen=true;G.shopChance=Math.min(1,(G.shopChance||BASE_SHOP_CHANCE)+SHOP_CHANCE_STEP);return 'bloodAltar';}
  const discoveryRoll=gameRandom();
  ...
  if(gameRandom()<churchChance){G.shopChance=Math.min(1,(G.shopChance||BASE_SHOP_CHANCE)+SHOP_CHANCE_STEP);return gameRandom()<0.5?'ordinaryChurch':'darkChurch';}
...
function openSquirrelNestEvent(){
  ...
  $('squirrel-search').onclick=()=>{
    const found=squirrelNestSearchGold();gainGold(found);SFX.coin();renderTop();
...
function addBleed(target,amount,source=null,effect='流血'){const result=addCappedStatusWithTrauma(target,'bleed',amount,8);if(target===G.battle&&(result.gained||result.traumaGained)&&source)rememberPlayerStatusSource('bleed',source,effect);return result;}
function addBurn(target,amount,source=null,effect='燒傷'){const result=addCappedStatusWithTrauma(target,'burn',amount,5);if(target===G.battle&&(result.gained||result.traumaGained)&&source)rememberPlayerStatusSource('burn',source,effect);return result;}
...
function squirrelSteal(enemy){
  ...
  log(`🐿️ ${enemy.name}偷走了 ${amt} 金幣！擊敗牠即可取回這一份贓物。`,'dmg');renderTop();return amt;
}
function recoverSquirrelGold(enemy){
  const amount=Math.max(0,Math.round(enemy&&enemy.stolenGold||0));if(!amount)return 0;
  G.gold+=amount;enemy.stolenGold=0;SFX.coin();log(`🐿️ 擊敗 ${enemy.name}，取回牠偷走的 ${amount} 金幣！`,'gd');renderTop();return amount;
}
...
function attackEnemy(dmg,opts={}){
  ...
  if(e.curhp<=0){
    ...
    if(e.type==='zombie'&&!e.revived&&!directExecution){
      ...
      log(`${e.name} 被擊倒！`,'good');recordEnemyDefeat(e);ensureTarget();
...
function defeatEnemyByPoison(e){
  ...
  e.curhp=0;recordEnemyDefeat(e);log(`☠ ${e.name}被中毒擊倒！`,'good');
  if(e.type==='squirrel')recoverSquirrelGold(e);
...
function endPlayerTurn(){
  ...
  setTimeout(()=>{
    ...
    if(b.burn>0){
      const burn=tickBurnStatus(b);losePlayerHp(burn.damage,statusDamageSource('burn'));
    ...
    if(bloodExamEnemy&&net>0){const result=addBleed(b,enemyStatusRaw(bloodExamEnemy,1),bloodExamEnemy,'血魔撕裂');if(result.traumaGained)log(`🩹 流血已滿：新增的 ${result.traumaGained} 層流血轉為創傷（目前 ${b.trauma} 層）。`,'dmg');else if(result.gained>0)log(`🩸 血魔考官的攻擊命中 HP：流血 +${result.gained}（目前 ${b.bleed}/8 層）。`,'dmg');}
    ...
    if(net>0)mimicHitEvents.forEach(event=>{
      if(event.action==='venomBite'){
        ...
        const result=addBleed(b,enemyStatusRaw(event.enemy,2),event.enemy,'撕裂長舌・流血');if(result.gained)log(`🩸 長舌傷及 HP：流血 +${result.gained}（目前 ${b.bleed}/8）。`,'dmg');if(result.traumaGained)log(`🩹 溢出流血轉為創傷 +${result.traumaGained}。`,'dmg');
      }else{
        const gained=addFracture(b,enemyStatusRaw(event.enemy,1));if(gained)log(`🦴 夾擊傷及 HP：斷骨 +${gained}（目前 ${b.fracture}/3）。`,'dmg');
      }
    });
    if(net>0&&pengBleed>0){const result=addBleed(b,enemyStatusRaw(pengStatusEnemy,pengBleed),pengStatusEnemy,`${enemyAttackEffect(pengStatusEnemy)}・流血`);if(result.gained)log(`🩸 鵬的攻擊附加流血 +${result.gained}（目前 ${b.bleed}/8）。`,'dmg');if(result.traumaGained)log(`🩹 溢出流血轉為創傷 +${result.traumaGained}（目前 ${b.trauma}）。`,'dmg');}
    if(net>0&&pengBurn>0){const result=addBurn(b,enemyStatusRaw(pengStatusEnemy,pengBurn),pengStatusEnemy,`${enemyAttackEffect(pengStatusEnemy)}・燒傷`);if(result.gained)log(`🔥 鵬的攻擊附加燒傷 +${result.gained}（目前 ${b.burn}/5）。`,'dmg');if(result.traumaGained)log(`🩹 溢出燒傷轉為創傷 +${result.traumaGained}（目前 ${b.trauma}）。`,'dmg');}
...
function enemyGuideData(e){
  const action=(name,desc)=>({name,desc}),simple=(passive='無特殊被動。')=>({passives:[passive],actions:[action('普通攻擊','造成目前顯示的攻擊傷害。')]});
  const guides={
    ...
    ghost:()=>({passives:['每第 3 回合進入無敵；無敵會使玩家攻擊完全無效。'],actions:[action('幽靈攻擊','非無敵回合造成 1.0 倍基礎攻擊傷害。'),action('無敵','第 3、6、9……回合生效；該回合仍依目前意圖行動。')]}),
    witch:()=>({passives:['第 5 回合固定改為施放劇毒。'],actions:[action('魔法攻擊','除第 5 回合外造成 1.0 倍基礎攻擊傷害。'),action('劇毒','第 5 回合不造成直接傷害，施加 2 層中毒；實際層數受狀態倍率與抗性影響。')]}),
    bear:()=>({passives:['第 1 回合與之後每第 5 回合的攻擊具有重壓效果。'],actions:[action('熊掌攻擊','造成 1.0 倍基礎攻擊傷害。'),action('重壓','在指定回合隨攻擊施加 3 層虛弱；實際層數受狀態倍率與抗性影響。')]}),
    platypus:()=>({passives:['第 1 回合與之後每第 3 回合的攻擊附加遲疑。'],actions:[action('鴨嘴獸攻擊','造成 1.0 倍基礎攻擊傷害。'),action('遲疑打擊','隨攻擊施加基礎 3 層遲疑，層數代表下一次行動可額外抽牌數。')]}),
    squirrel:()=>({passives:[`第 ${squirrelEscapeTurns(G.floor)} 回合結束後逃跑。每隻松鼠分別保存自己偷走的金錢；被擊敗時返還該隻松鼠的贓款。`,`目前攜帶贓款：${Math.max(0,e.stolenGold||0)} 金幣。`],actions:[action('偷竊攻擊','第 1 回合攻擊後獨立偷取金錢。'),action('普通攻擊','其餘回合造成 1.0 倍基礎攻擊傷害。'),action('逃跑','逃跑時間到達後帶走仍持有的贓款並結束戰鬥。')]}),
    ...
    dropbear:()=>({passives:['固定休息 2 回合，第 3 回合發動猛撲，之後重複。'],actions:[action('蓄力休息','不攻擊。'),action('蓄力猛撲','造成高額基礎傷害，並施加 2 層中毒與 4 層虛弱。')]}),
    ...
    eagle:()=>({passives:[`擁有 ${e.maxEvasion||eagleGrowth(G.floor).maxEvasion} 層閃避。16 點以下的攻擊會消耗 1 層閃避而失效；17～19 點可命中；20／21 點命中。`,'成功閃避後，下一次行動改為俯衝反擊。'],actions:[action('利爪攻擊','造成 1.0 倍基礎攻擊傷害。'),action('俯衝反擊',`成功閃避後使用，造成 ${eagleGrowth(G.floor).diveMult.toFixed(1)} 倍傷害。`)]}),
    robot:()=>{const rg=robotGrowth(G.floor);return {passives:['依「火焰噴射 → 電力充能 → 電弧放電 → 過熱冷卻」循環行動。'],actions:[action('火焰噴射',`造成 1.0 倍傷害；傷及 HP 時施加 ${rg.burn} 層燒傷。`),action('電力充能',`不攻擊；高樓層時獲得 ${rg.chargeShield} 護盾。`),action('電弧放電',`造成 1.4 倍基礎傷害，吸收目前蓄勢的 ${Math.round(rg.focusRate*100)}%，最多 ${rg.focusCap} 點，將吸收量加入傷害並清除玩家全部蓄勢。`),action('過熱冷卻','不攻擊；該回合受到的傷害 ×1.4。')]};},
    ...
    cyclops:()=>({passives:[`目前循環長度 ${cyclopsGrowth(G.floor).cycleLength} 回合。凝視回合受到 18～21 點成功攻擊會中斷下一次粉碎重擊。`],actions:[action('巨棒揮擊','造成 1.0 倍基礎攻擊傷害。'),action('獨眼凝視','不攻擊；設定下一回合粉碎重擊，可被指定點數命中中斷。'),action('粉碎重擊','造成 2.0 倍傷害；實際傷及 HP 時施加 1 層斷骨。')]}),
    ...
    werewolf:()=>({passives:['攻擊與治療會依玩家目前流血層數增強。'],actions:[action('狼爪','造成 1.0 倍傷害；實際傷及 HP 時施加 2 層流血。'),action('嗅血撕咬',`倍率為 1.2＋每層流血 0.05，最高 ×1.5；目前 ×${werewolfBiteMultiplier().toFixed(2)}。`),action('舔舐傷口',`不攻擊；回復最大生命的 10%＋每層流血 1%，最高 18%；目前 ${Math.round(werewolfHealRate()*100)}%。`)]}),
    ...
    punishmentGargoyle:()=>{const vr=enemyVirulenceRule(e,'toxicWhip');return {passives:['50% 負面狀態抗性，永久護盾。死亡時使玩家目前中毒層數翻倍，並使狂信 −5。'],actions:[action('刑罰石爪','造成 1.0 倍基礎攻擊傷害。'),action('毒刑','造成 1.1 倍傷害；傷及 HP 時施加 2 層中毒。'),action('石像封印','不攻擊；最多封鎖 1 項主動或被動技能。'),action('毒鞭',`造成 1.2 倍傷害；傷及 HP 時施加 3 層中毒。每 ${vr.threshold} 層中毒轉化 ${vr.yield} 層猛毒。`)]};},
    cultLeader:()=>({passives:[`終極 Boss 第一階段，70% 負面狀態抗性。兩尊石像鬼存活時各提供 25% 減傷；教宗死亡時石像鬼一同死亡。`,`狂信開局 5 層、上限 20；每層使攻擊 +2%。所有玩家強化在本階段被封印。戒律懲罰會放大玩家負面狀態、造成遞增傷害，並使敵方全體回復 5% 與獲得 10% 永久護盾。`],actions:[action('黑經誦讀','造成 1.0 倍以上傷害；傷及 HP 時施加腐敗，滿狂信時層數提高。'),action('盲目佈道','造成 1.0 倍以上傷害並施加致盲，滿狂信時層數提高。'),action('敗血儀式','造成 1.15 倍傷害；傷及 HP 時施加敗血。'),action('汲血','造成 1.2 倍傷害，回復實際 HP 傷害的 50%。'),action('褻瀆共融','造成 1.1 倍傷害，並依玩家持有的負面狀態種類回復更多生命。'),action('黑曜赦令','石像鬼存活時使用；不攻擊，敵方全體獲得永久護盾，暫時歸還玩家強化 1 回合，石像鬼下一次傷害 ×1.3。')]}),
    cthulhu:()=>({passives:[`終極 Boss 第二階段，70% 負面狀態抗性，生命上限等於第一階段教宗與兩尊石像鬼生命上限總和。繼承並凍結第一階段狂信，每層攻擊 +2%。`,`深淵距離開局 10、上限 20，每回合自然 −1；歸零時玩家 HP 歸零，復活後距離從 5 開始。戒律烙印永久存在，爆牌直接增加 2 層。`],actions:[action('萬觸撕裂','造成 1.3 倍傷害；傷及 HP 時施加 5 層流血與 1 層斷骨。'),action('不可名狀的凝視','造成至少 1.0 倍傷害，施加幻覺與精神錯亂。'),action('深淵震鳴','造成 1.2 倍傷害；除自然托拽外再使深淵距離 −1。'),action('群星囈語','造成至少 1.0 倍傷害，施加致盲與遲疑。'),action('深海威壓','造成 1.1 倍傷害，施加虛弱；傷及 HP 時施加腐敗。'),action('深淵再生','唯一無攻擊動作；回復 6% 最大生命並獲得 12% 最大生命的永久護盾。')]}),
    inquisitorMounted:()=>({passives:['終極 Boss 第一階段，70% 負面狀態抗性與永久 30% 減傷。馬勢開局 5、上限 25；低於 10 時回復速度提高，每層使騎馬攻擊 +3%。','每次玩家攻擊行動傷及審判長陣營、施加負面狀態、爆牌或擊殺聖騎士都會增加罪證；罪證無上限。'],actions:[action('騎槍突刺','造成 1.2 倍傷害並受馬勢與戰吼加成；傷及 HP 時施加 2 層流血。'),action('戰馬踐踏','造成 1.05 倍傷害並受馬勢與戰吼加成；傷及 HP 時施加 1 層斷骨。'),action('宣讀罪狀','不攻擊；罪證 +1、施加虛弱並獲得最大生命 12% 護盾。'),action('舉槍裁決','裁決衝鋒準備；不攻擊，全體驅散 50% 負面狀態並獲得最大生命 30% 戰馬護甲，聖騎士同步戰吼。'),action('裁決衝鋒','造成 2.4 倍傷害並受馬勢與戰吼加成，具有高破防；傷及 HP 時施加流血與斷骨。')]}),
    inquisitor:()=>({passives:['終極 Boss 第二階段，70% 負面狀態抗性與永久 30% 減傷。第二型態最大生命＝第一型態原始生命 ×（1.5＋剩餘馬勢 ×0.05）。','罪證按 1：100 轉化並形成罪惡值；罪惡值無倍率上限，會提高審判長與聖騎士傷害。後續再犯以原罪證效果 80% 增加，但不超過開局罪惡值上限。'],actions:[action('下馬整備','轉階後停頓 1 回合，下回合固定審判。'),action('斷罪劍','造成 1.2 倍傷害並受罪惡值加成；傷及 HP 時施加 2 層流血。'),action('火刑宣告','造成 1.1 倍傷害並受罪惡值加成；傷及 HP 時施加 3 層燒傷與 1 層腐敗。'),action('信仰拷問','造成 1.0 倍傷害；傷及 HP 時奪取 2 控制值，控制值為 0 時改施加 2 層虛弱。'),action('沒收異端財物','不攻擊；奪取金錢並將其中 25% 轉化為護盾。'),action('審判','無前搖，造成 1.4 倍傷害並使用較高罪惡值加成；傷及 HP 時施加斷骨，存活聖騎士同步使用裁決斬擊。')]}),
  };
...
function renderEnemyInfo(){
  ...
  $('enemy-info-list').innerHTML=b.enemies.map(e=>{
    const guide=enemyGuideData(e),current=enemyAttackEffect(e),statuses=[e.statusResist>0?`負面狀態抗性 ${Math.round(e.statusResist*100)}%`:'',e.shield>0?`護盾 ${e.shield}`:'',e.evasion>0?`閃避 ${e.evasion}/${e.maxEvasion}`:'',e.poison>0?`中毒 ${e.poison}`:'',e.virulence>0?`猛毒 ${e.virulence}`:'',e.bleed>0?`流血 ${e.bleed}`:'',e.burn>0?`燒傷 ${e.burn}`:'',e.trauma>0?`創傷 ${e.trauma}`:''].filter(Boolean);
    return `<section class="enemy-info-card"><div class="enemy-info-title"><span>${escapeHtml(e.name)}</span><span>${e.curhp>0?'存活':'已倒下'}</span></div><div class="enemy-info-stats">HP ${Math.max(0,e.curhp)}/${e.maxhp}｜基礎攻擊範圍 ${e.atk[0]}～${e.atk[1]}｜目前動作 ${escapeHtml(current)}${Number.isFinite(e.nextDmg)?`｜預計傷害 ${e.nextDmg}`:''}${statuses.length?`<br>${escapeHtml(statuses.join('｜'))}`:''}</div><div class="enemy-info-group"><div>被動與特殊規則</div>${guide.passives.map(text=>`<div class="enemy-info-passive">${escapeHtml(text)}</div>`).join('')}</div><div class="enemy-info-group"><div>所有動作</div>${guide.actions.map(item=>`<div class="enemy-info-row"><b>${escapeHtml(item.name)}</b><span>${escapeHtml(item.desc)}</span></div>`).join('')}</div></section>`;
  }).join('');
}
